...
int IxIndexHandle::compare_key(const char *lhs, const char *rhs) const {
    return ix_compare(lhs, rhs, file_hdr_->col_types_, file_hdr_->col_lens_);
}

size_t IxIndexHandle::lower_bound_slot(const char *key) const {
    auto it = std::lower_bound(entries_.begin(), entries_.end(), key,
                               [this](const auto &entry, const char *rhs) {
                                   return compare_key(entry.first.data(), rhs) < 0;
                               });
    return static_cast<size_t>(it - entries_.begin());
}

size_t IxIndexHandle::upper_bound_slot(const char *key) const {
    auto it = std::upper_bound(entries_.begin(), entries_.end(), key,
                               [this](const char *lhs, const auto &entry) {
                                   return compare_key(lhs, entry.first.data()) < 0;
                               });
    return static_cast<size_t>(it - entries_.begin());
}

bool IxIndexHandle::contains_key(const char *key) const {
    size_t pos = lower_bound_slot(key);
    return pos < entries_.size() && compare_key(entries_[pos].first.data(), key) == 0;
}
...
bool IxIndexHandle::get_value(const char *key, std::vector<Rid> *result, Transaction *transaction) {
    ...
    size_t pos = lower_bound_slot(key);
    if (pos >= entries_.size() || compare_key(entries_[pos].first.data(), key) != 0) {
        return false;
    }
    result->push_back(entries_[pos].second);
    return true;
}
...
page_id_t IxIndexHandle::insert_entry(const char *key, const Rid &value, Transaction *transaction) {
    ...
    std::vector<char> key_buf(file_hdr_->col_tot_len_);
    memcpy(key_buf.data(), key, file_hdr_->col_tot_len_);
    entries_.insert(entries_.begin() + static_cast<std::ptrdiff_t>(pos), {std::move(key_buf), value});
...
Rid IxIndexHandle::get_rid(const Iid &iid) const {
    if (iid.slot_no < 0 || static_cast<size_t>(iid.slot_no) >= entries_.size()) {
        throw IndexEntryNotFoundError();
    }
    return entries_[iid.slot_no].second;
}
...
Iid IxIndexHandle::lower_bound(const char *key) {
    ...
    return Iid{0, static_cast<int>(lower_bound_slot(key))};
}
...
Iid IxIndexHandle::upper_bound(const char *key) {
    ...
    return Iid{0, static_cast<int>(upper_bound_slot(key))};
}
