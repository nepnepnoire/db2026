...
class RmFileHandle {      
   ...
   public:
    ...
    bool is_record(const Rid &rid) const {
        ...
        bool exist = Bitmap::is_set(page_handle.bitmap, rid.slot_no);  // page的slot_no位置上是否有record
    ...
    void recover_insert_record(const Rid &rid, char *buf, lsn_t page_lsn = INVALID_LSN);

    void recover_delete_record(const Rid &rid, lsn_t page_lsn = INVALID_LSN);

    void recover_update_record(const Rid &rid, char *buf, lsn_t page_lsn = INVALID_LSN);

    // 恢复阶段（undo/补偿）显式推进指定页面的 page_lsn，保证页面内容与 page_lsn 一致、且不回退
    void recover_stamp_page_lsn(const Rid &rid, lsn_t page_lsn);

    void flush();


    void flush_page(const Rid &rid);
