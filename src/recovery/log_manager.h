...
public:
    ...
    void append_begin(txn_id_t txn_id);
    void append_commit(txn_id_t txn_id);
    void append_abort(txn_id_t txn_id);
    lsn_t append_insert(txn_id_t txn_id, const std::string &table_name, const Rid &rid, const char *data, int size);
    lsn_t append_delete(txn_id_t txn_id, const std::string &table_name, const Rid &rid, const char *old_data, int size);
    lsn_t append_update(txn_id_t txn_id, const std::string &table_name, const Rid &rid,
                        const char *old_data, const char *new_data, int size);
    // advance_to_active_only=true 时（存在活跃事务的 fuzzy checkpoint），
    // restart_offset 只推进到“最早仍活跃事务的 BEGIN”处，保证 loser 的 undo 记录仍在扫描范围内；
    // 否则（无活跃事务）推进到 checkpoint 记录处。
    long create_static_checkpoint_record(bool advance_to_active_only = false);
    void advance_next_lsn(lsn_t max_seen_lsn);
    static long read_restart_offset();
    static void write_restart_offset(long offset);
    static long current_log_size();
    // 扫描日志，返回“最早仍活跃（有 BEGIN 但无 COMMIT/ABORT）事务”的 BEGIN 字节偏移；
    // 若不存在活跃事务则返回 fallback_offset。
    static long compute_min_active_begin_offset(long fallback_offset);


    LogBuffer* get_log_buffer() { return &log_buffer_; }

private:
    lsn_t append_recovery_line(const std::string &line);
    lsn_t append_recovery_op_line(const std::string &tag, txn_id_t txn_id, const std::string &table_name,
